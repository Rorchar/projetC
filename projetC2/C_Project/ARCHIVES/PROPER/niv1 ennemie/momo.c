...
int main(){
    ...
    int g=0;
    for(g=0;g<nbE;g++){
        Tank_E=TankE[setE[g]-1];
        tank_E(Tank_E,ren,xTankE[g],yTankE[g]);
    }
    Tank_E=TankE[4-1];
    tank_E(Tank_E,ren,435,535);
    ...
    while( ON == 1 ) {
        ...
        while (action==1){    
            ...
            int f=0;
            for (f=0;f<nbE;f++){
                if (((x==xTankE[f] &&y==yTankE[f]) || (x==xTankE[f]+10*dx[setE[f]-1] && y==yTankE[f]+10*dy[setE[f]-1]) || (x==xTankE[f]+20*dx[setE[f]-1] && y==yTankE[f]+20*dy[setE[f]-1]) || (x==xTankE[f]+30*dx[setE[f]-1] && y==yTankE[f]+30*dy[setE[f]-1])) && StatutE[f]==0){
