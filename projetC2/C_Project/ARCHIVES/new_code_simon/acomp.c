...
int main(int argc, char *argv[]){
    ...
    while( ON == 1 ) {
		...
		while (action==1){	
			...
			switch(set){
				case (1):Tank=t_1;break;
				case (2):Tank=t_2;break;
				case (3):Tank=t_3;break;
				case (4):Tank=t_4;break;
				case (5):break;
				case (6):Tank=t_6;break;
				case (7):Tank=t_7;break;
				case (8):Tank=t_8;break;
				case (9):Tank=t_9;break;
			}
			for(i=0;i<10;i++){
				fond_terre(renderer);
			...
			for(nbj=0;nbj<nb;nbj++){
					if(x==xcible[nbj] && y==ycible[nbj]){
						SDL_ShowSimpleMessageBox(0, "jojo", "le rigolo ", window);
					}
				}
...
//gcc copiede000save.c -o 000save.h -Wall -lSDL2_image -lSDL2
//Sources:
